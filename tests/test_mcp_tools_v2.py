...
class TestDeprecatedTools:
    """Test deprecated V1 delegation tools with warnings."""
    
    @pytest.mark.asyncio
    async def test_delegate_work_shows_deprecation_warning(self):
        """Test that delegate_work shows deprecation warning."""
        with patch('ct_dev_agent_orchestrator_mcp.server.delegation_service') as mock_service:
            mock_response = MagicMock()
            mock_response.delegation_id = "del-123"
            mock_response.agent_id = "agent-456"
            mock_response.status = "delegated"
            mock_response.message = "Work delegated"
            mock_service.delegate = AsyncMock(return_value=mock_response)
            
            result = await call_tool("delegate_work", {
                "task_id": "task-123",
                "agent_role": "backend_specialist",
                "instructions": "Test work"
            })
            
            assert len(result) == 1
            assert "⚠️  WARNING: delegate_work is DEPRECATED. Use spawn_agent instead." in result[0].text
            assert "✓ Work delegated successfully" in result[0].text
    
    @pytest.mark.asyncio
    async def test_get_delegation_status_shows_deprecation_warning(self):
        """Test that get_delegation_status shows deprecation warning."""
        with patch('ct_dev_agent_orchestrator_mcp.server.delegation_service') as mock_service:
            mock_status = {
                'id': 'del-123',
                'agent_id': 'agent-456',
            ...
            result = await call_tool("get_delegation_status", {
                "delegation_id": "del-123"
            })
            
            assert len(result) == 1
            assert "⚠️  WARNING: get_delegation_status is DEPRECATED. Use query_session instead." in result[0].text
            assert "Delegation Status:" in result[0].text
    
    @pytest.mark.asyncio
    async def test_get_delegation_result_shows_deprecation_warning(self):
        """Test that get_delegation_result shows deprecation warning."""
        with patch('ct_dev_agent_orchestrator_mcp.server.delegation_service') as mock_service:
            ...
            mock_result.scope_deviation = None
            mock_service.get_result = AsyncMock(return_value=mock_result)
            
            result = await call_tool("get_delegation_result", {
                "delegation_id": "del-123"
            })
            
            assert len(result) == 1
            assert "⚠️  WARNING: get_delegation_result is DEPRECATED. Use get_agent_output instead." in result[0].text
            assert "Delegation Result:" in result[0].text
    
    @pytest.mark.asyncio
    async def test_list_delegations_shows_deprecation_warning(self):
        """Test that list_delegations shows deprecation warning."""
        with patch('ct_dev_agent_orchestrator_mcp.server.delegation_service') as mock_service:
            mock_delegations = [
                {
                    'id': 'del-123',
            ...
            mock_service.list_delegations = AsyncMock(return_value=mock_delegations)
            
            result = await call_tool("list_delegations", {})
            
            assert len(result) == 1
            assert "⚠️  WARNING: list_delegations is DEPRECATED. Use list_active_sessions instead." in result[0].text
            assert "Delegations (1):" in result[0].text
    
    @pytest.mark.asyncio 
    async def test_cancel_delegation_shows_deprecation_warning(self):
        """Test that cancel_delegation shows deprecation warning."""
        with patch('ct_dev_agent_orchestrator_mcp.server.delegation_service') as mock_service:
            mock_service.cancel_delegation = AsyncMock(return_value=True)
            
            result = await call_tool("cancel_delegation", {
                "delegation_id": "del-123"
            })
            
            assert len(result) == 1
            assert "⚠️  WARNING: cancel_delegation is DEPRECATED. Use stop_agent instead." in result[0].text
            assert "✓ Delegation del-123 cancelled" in result[0].text
